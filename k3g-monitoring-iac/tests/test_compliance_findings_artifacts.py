"""Tests for compliance findings artifacts."""
...
def test_compare_generates_findings_artifacts(client, jobs_base, monkeypatch):
    ...
    data = response.json()
    assert data["decision"] in {"COMPLIANCE_COMPARE_COMPLETED", "COMPLIANCE_COMPARE_COMPLETED_WITH_FINDINGS"}

    results_dir = jobs_base / job["job_id"] / "comparison"
    assert (results_dir / "compliance-comparison-result.json").exists()
    assert (results_dir / "COMPLIANCE-COMPARISON-RESULT.md").exists()
    assert (results_dir / "devices" / "1890" / "compliance-findings.json").exists()
    assert (results_dir / "devices" / "1890" / "COMPLIANCE-FINDINGS.md").exists()
    assert data["comparison_result"]["safety"]["netbox_write"] is False


def test_compare_no_network_or_write_calls(client, jobs_base, monkeypatch):
    job = _prepare_collection_artifacts(client, jobs_base, monkeypatch)
    with patch("webui.services.compliance_jobs.JOBS_BASE", jobs_base), patch(
        "webui.services.compliance_huawei_ne8000_parser.JOBS_BASE", jobs_base
    ):
