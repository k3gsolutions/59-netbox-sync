...
- `week1-metadata-collection-template.csv` is the source of truth for pending items
- `week1-responses/` stores local team CSVs generated by the Web UI modal
- `week1-responses/audit/` stores append-only audit JSON for each local save
- `/reports/download` serves safe CSV/JSON/TXT/LOG artifacts and still blocks sensitive paths
- The modal-based editor is documented in `docs/62-webui-response-form.md`
- PT-BR UX copy guidance is documented in `docs/66-webui-ptbr-ux-copy.md`
- Real Week 1 activation is documented in `docs/67-real-week1-activation-flow.md`
- The validation dashboard is documented in `docs/64-webui-response-validation-dashboard.md`
- UAT cleanup/readiness is documented in `docs/63-week1-uat-response-handling.md`
...
- Final validation: `REAL-WEEK1-FINAL-VALIDATION.md`
- No NetBox writes, no apply, no `/sync`, no automatic ApprovalRecords
